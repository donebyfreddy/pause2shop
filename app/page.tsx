...
async function loadLandingData(): Promise<LandingData> {
  ...
  return {
    stats: [
      { label: "Fuentes registradas", value: num(connectors?.summary.total) },
      { label: "Productos en catálogo", value: num(overview?.catalog.totalProducts) },
      {
        label: "Con embedding visual",
        value: overview ? `${overview.embeddings.coverage}%` : "—",
      },
      { label: "Categorías detectables", value: "8" },
    ],
